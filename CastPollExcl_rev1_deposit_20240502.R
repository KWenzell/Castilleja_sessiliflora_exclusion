...
setwd("~/CASE_Rdocs/Cast_pollinators/CastPhD2.2_MS/Submission_1/submissionDeposit20240310/")
...
vc <- ggplot(vis, aes(x=factor(pop, level=popOrd), y=pgVisit, fill=factor(pollFxGrp2, level= pollOrd)))+
  ...
  scale_fill_manual(values=c('hummingbird'="tomato2", 
                             'small_bee'="orange", 'bumblebee'="gold1", 
                             "otherD" = "salmon", 
...
#subsetted datasets (below) have no repeat years, so year RE not included
...
#model doesn't perform well with betabinomial distribution, so use binomial
...
sim_resid_glmmTMB<- simulateResiduals(m, 1000)
plot(sim_resid_glmmTMB)
#decide to proceed based on inspecting plot and to keep analyses comparable
testZeroInflation(sim_resid_glmmTMB)
#NS
...
plot(sim_resid_glmmTMB)
testZeroInflation(sim_resid_glmmTMB)
#tested binomial model, DHARMa assessments performed worse than betabinomial model
